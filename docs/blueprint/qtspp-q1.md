...
the *true* diagonal sequence (paper §3), honestly separated from the guessed
sequence `B′` (§5.1–5.3).

**Key distinction (read 2026-09-24, arXiv:0906.1018 §§3, 5.1–5.3):** the
paper's §5.3 computation proves the diagonal statement for the *guessed*
sequence `B′(n,j)` — defined by a 1.6 MB five-generator bivariate
Gröbner/∂-finite description plus initial data — not directly for the true
normalized cofactors `B(n,j)`. For the true diagonal, `B(n,n) = 1` is the
normalization argument: deleting the last row/column of `A_n` gives `A_{n-1}`
definitionally, so `B(n,n) = det(A_{n-1})/det(A_{n-1}) = 1` wherever the
determinant is nonzero. The seven small determinant values are exact integer
arithmetic, not ∂-finite output.

**Delivered — `FragileProofAudit/QTSPP/TrueDiagonal.lean`:**
- `diagMinor n` — the `(n+1,n+1)` minor (delete last row/column);
  `diagMinor_eq : diagMinor n = tsppMatrix n` (entries line up definitionally).
- `trueDiag n = (diagMinor n).det / (tsppMatrix n).det` — the paper's
  `B(n+1,n+1)` (sign `(-1)^{2(n+1)} = 1`).
- `trueDiag_eq_one_of_det_ne` — the normalization argument.
- `trueDiag_hinit : ∀ i < 7, trueDiag i = 1` — finite initialization for the
  *true* normalized cofactor (not the guessed `B′`), from seven
  inverse-matrix certificates (`invCert1`–`invCert6` plus the `Fin 0` case by
  `Matrix.det_fin_zero`), each product `A_k * B_k = 1` verified by
  `native_decide` (kernel `decide` gets stuck on `Fin` literal reduction;
  see linter warning); `det_ne_zero_of_mul_eq_one` turns each into
  `det ≠ 0`. Certificates generated by exact rational Gauss-Jordan
  (`~/workspace/qtspp/gen_inverse_certs.py`).
- `cert_hrec_of_diag_one` — hrec reduced: for any `DiagonalCertificate`, a
  constantly-1 diagonal is annihilated by the right-factor telescope.
- `trueDiag_hrec_of_det_ne`, `true_diagonal_of_det_ne` — the full `B(n,n)=1`
  for the true diagonal, conditional on `∀ n, det(A_n) ≠ 0` (exactly the
  content the norm identity (3.3), `det = Nice(n)²`, delivers).

**Delivered — gate `scripts/gates/qtspp_diagonal.py`** (standalone control gate,
*not* a verdict-lock entry; the 25-gate verdict lock is untouched). Finite/control
replay, fail-closed:
1. SHA-256 pin of Koutschan's `Bnn_op_0_a23r.txt`
   (`d12c1027…b8570`; vendored at `incoming/qtspp-q1/Bnn_op_0_a23r.txt`).
2. Operator transcription fidelity: all 8 `koutschanP` polynomials parsed
   from the Lean file equal the source terms (sympy).
3. True-diagonal replay from the paper formula: `det(A_n) ≠ 0`,
   `minor(n+1,n+1) = A_n`, `B(n,n) = 1` for `n ≤ 12`.
4. Inverse-cert transcription: `A_k * invCert_k = I` over ℚ, exact, `k=1..6`.
5. hrec finite replay: `∑ p_i(nn)·B(nn+i+1,nn+i+1) = 0` for `nn = 0..25`.

**Honest position:** finite initialization for the true diagonal is formalized
in Lean (`native_decide`-checked inverse certificates). hrec for the true
diagonal is exactly `∀ n, det(A_n) ≠ 0`. The ∂-finite-substitution route for
the *guessed* `B′` — the five Gröbner generators, bivariate initial data,
and substitution certificate — remains the multi-week infrastructure item;
author contact is the plausible route to those artifacts. Nothing here is
presented as closing `B′`'s hrec.
